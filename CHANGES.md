...
- Convert bitonal pages across several processes, and stop paying for per-page work that was being discarded. `api.bitonal` takes a `workers` parameter (default 1, so existing callers convert inline exactly as before) that splits the document into one contiguous page range per worker and merges the chunks back in page order; `ocr.bitonal_convert` takes the same parameter, and `blackletter process` exposes it as `--bitonal-workers`. Callers choose the count because only they know what the machine was granted: rasterisation is memory-bandwidth bound, so throughput flattens well before one worker per core. Independently of parallelism, three per-page fixes: each page is thresholded with `np.packbits` in a single pass instead of materialising a full 8-bit image for PIL to threshold a second time; the TIFF handed to `insert_image` is no longer CCITT G4 compressed, because MuPDF decodes whatever it is given and re-encodes the embedded image as `/FlateDecode`, so that pass cost time and produced a byte-identical PDF; and ICC colour management is disabled for the render, which cannot meaningfully change a page about to be thresholded to one bit. On a 942-page volume at 200 DPI the three fixes are worth 1.48x on their own, and with 6 workers on an 8-CPU machine the conversion drops from 414s to 113s, with output identical in size and within 0.08% of pixels. The per-page logic also no longer exists in three places: the copy that lived inside the `_BITONAL_WORKER_SCRIPT` string literal is gone, and the parallel path goes through `tasks.bitonal_chunk` like every other caller, so a fix to the renderer can no longer miss it. The pool pins the `spawn` start method, the only one available on every platform and already the default on macOS, so a script that calls this at module scope with `workers` above 1 needs the usual `if __name__ == "__main__":` guard (#71)
- Add a `bitonal_workers` parameter to `process()`, which builds its argument namespace by hand and would otherwise omit it, so the Python entry point could reach `--bitonal-workers` only from the CLI. This is the same gap `text_layer` and `ocr` had in 0.2.0, and it fails silently rather than loudly: `cmd_process` reads the count through a `getattr` default of 1, so a caller got correct output and none of the speedup (#71)
