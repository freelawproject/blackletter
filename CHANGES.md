...
- Add an `engine` option to the OCR step (`"tesseract"` default or `"paddle"`) on `ocr_pdf()`, `api.ocr()`, and `ocr_chunk()`. `engine="paddle"` runs OCR through a native PaddleOCR ocrmypdf engine (`blackletter/paddle_ocr.py`), cached per process, `enable_mkldnn=False`, GPU auto-detected; requires the `[analyze]` extra. It uses the PP-OCRv5 *server* detector and recognizer (shared with `analyze`, so weights are baked once and line-box quality for redaction-rect tightening plus text accuracy are preserved). The engine runs serially in the calling thread (a `SerialExecutor` plugin), because a Paddle CUDA context created on an ocrmypdf worker thread coexists badly with a pre-existing torch CUDA context and hangs the first page. Recognized lines are sorted into column-aware reading order (a recursive XY-cut that detects column gutters before stacked blocks) and grouped into per-column paragraphs, so the text layer of a multi-column page selects column-by-column instead of straight across. It also logs per-page progress (`paddle: recognizing ...` / `... -> N lines in Xs`)
- Add a `progress_callback(current, total, message)` parameter to `api.ocr()` (matching `api.bitonal()`), so per-page OCR progress can be relayed without a downstream copy of the function
