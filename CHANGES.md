...
- Lower the `PAGE_HEADER` and `HEADNOTE_BRACKET` confidence gates from 0.50 to 0.30 and add a 0.50 gate for `EDITORIAL`, measured on the golden val/test set: everything bl-warm places between 0.25 and 0.50 for the two lowered classes is a true positive the old gate silently dropped, while `EDITORIAL`'s one harmful false positive sits below 0.50 and every true positive above it (#73)
